...
import { validateParams } from '../../auth/interfaces/middlewares/validate-body'
 ...
 * Todas requieren autenticación y validación de UUID
 */

// POST /api/workshops/:id/stripe/connect - Iniciar conexión de Stripe
router.post('/:id/stripe/connect', validateParams(UuidParamSchema), verifyToken, initiateStripeConnect)

// POST /api/workshops/:id/stripe/refresh-onboarding - Regenerar link de onboarding
router.post('/:id/stripe/refresh-onboarding', validateParams(UuidParamSchema), verifyToken, refreshOnboardingLink)

// GET /api/workshops/:id/stripe/status - Estado de la cuenta conectada
router.get('/:id/stripe/status', validateParams(UuidParamSchema), getStripeAccountStatus)

// POST /api/workshops/:id/stripe/dashboard-link - Link al dashboard de Stripe
router.post('/:id/stripe/dashboard-link', validateParams(UuidParamSchema), verifyToken, getStripeDashboardLink)

// POST /api/workshops/:id/stripe/recover - Recuperar y asociar cuenta de Stripe existente (por metadata)
router.post('/:id/stripe/recover', validateParams(UuidParamSchema), verifyToken, recoverStripeAccount)

// POST /api/workshops/:id/stripe/recover-by-email - Recuperar cuenta de Stripe por email del owner
router.post('/:id/stripe/recover-by-email', validateParams(UuidParamSchema), verifyToken, recoverStripeAccountByEmail)

// GET /api/workshops/:id/debug - Info de diagnóstico del workshop
router.get('/:id/debug', validateParams(UuidParamSchema), getWorkshopDebugInfo)
